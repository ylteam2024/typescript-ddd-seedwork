...
import { IllegalStateException } from "src"

describe("Test NotificationReader ", () => {
  it("Test reading", () => {
    const plain = { event: { a: 'a', b: 'b', c: { a: true, d: '2007-03-01T13:00:00Z' } } }
    ...
    expect(reader.eventDateValue('/c/d')).toBe(Date.parse(plain['event']['c']['d']))
  })
  it("Test abnormal input", () => {
    const plain = { a: 'a' }
    expect(() => NotificationMessageReader.read(JSON.stringify(plain))).toThrowError(IllegalStateException)
  })
})
